...
// ErrorHandler는 에러 처리 전략을 정의하는 인터페이스입니다.
type ErrorHandler interface {
	Handle(err error)
}

// FatalErrorHandler는 log.Fatal을 사용하는 기본 에러 핸들러입니다.
type FatalErrorHandler struct{}

// Handle은 에러가 있을 경우 log.Fatal을 호출하여 프로세스를 종료합니다.
...
// SetErrorHandler는 에러 핸들러를 설정합니다 (주로 테스트용).
func SetErrorHandler(handler ErrorHandler) {
	errorHandler = handler
}

// ResetErrorHandler는 에러 핸들러를 기본값(FatalErrorHandler)으로 복원합니다.
func ResetErrorHandler() {
	errorHandler = &FatalErrorHandler{}
}

// CheckErr는 에러를 확인하고 설정된 핸들러를 통해 처리합니다.
