...
func init() {
	task.Register(TidJdc, &task.Config{
		...
		NewTask: func(instanceID task.InstanceID, req *task.SubmitRequest, appConfig *config.AppConfig) (task.Handler, error) {
			...
			t.SetFetcher(task.NewRetryFetcher(task.NewHTTPFetcher(), appConfig.HTTPRetry.MaxRetries, retryDelay, 30*time.Second))
...
func (t *jdcTask) scrapeOnlineEducationCourses(url string) ([]*jdcOnlineEducationCourse, error) {
	...
	err = task.ScrapeHTML(t.GetFetcher(), url, "#content > ul.prdt-list2 > li > a.link", func(i int, s *goquery.Selection) bool {
	...
	if err != nil {
		...
		if sel, _ := task.FetchHTMLSelection(t.GetFetcher(), url, "#content > div.no-data2"); sel != nil {
			return nil, nil
		}
...
func (t *jdcTask) scrapeOnlineEducationCourseCurriculums(url string, curriculumWebScrapeDoneC chan<- []*jdcOnlineEducationCourse) {
	...
	err := task.ScrapeHTML(t.GetFetcher(), fmt.Sprintf("%sproduct/%s", jdcBaseURL, url), "table.prdt-tbl > tbody > tr", func(i int, s *goquery.Selection) bool {
		// 강의목록 컬럼 개수를 확인한다.
		as := s.Find("td")
		if as.Length() != 3 {
