...
type NotifyMessageRequest struct {
	...
	ApplicationID string `json:"application_id" form:"application_id" query:"application_id" validate:"required" example:"my-app"`
	// 알림 메시지 내용 (최대 4096자, 마크다운 형식 지원)
	Message string `json:"message" form:"message" query:"message" validate:"required,min=1,max=4096" example:"서버에서 중요한 이벤트가 발생했습니다."`
