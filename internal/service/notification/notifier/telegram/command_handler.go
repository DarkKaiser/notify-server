...
func (n *telegramNotifier) submitTask(serviceStopCtx context.Context, command botCommand) {
	...
	//    - 부모 컨텍스트(serviceStopCtx)를 상속하여 서비스 종료 시그널과 생명주기를 일치시킵니다.
	//    - 사용자의 실시간 요청은 즉시성이 중요하므로, 서비스 종료 중에는
	//      새로운 작업을 받지 않는 것이 적절합니다. 서비스가 종료되면 대기 중인 Submit도 즉시 취소됩니다.
	//    - 스케줄러와 달리, 텔레그램 봇은 사용자가 직접 명령을 입력하는 상황이므로
	//      서비스 종료 시 "서비스 점검 중" 등의 안내를 제공하거나 요청을 거부하는 것이 사용자 경험상 자연스럽습니다.
	//    - 반면 스케줄러는 이미 트리거된 배치 작업을 완료해야 데이터 정합성이 보장되므로 Background()를 사용합니다.
	//
	// 2. 3초 타임아웃 적용 이유:
