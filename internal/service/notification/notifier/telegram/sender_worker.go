...
func (n *telegramNotifier) sendNotifications(serviceStopCtx context.Context) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			// ─────────────────────────────────────────────────────────────────
			// [중요] 명시적 종료 처리: Silent Failure 방지
			// ─────────────────────────────────────────────────────────────────
			// Sender 고루틴이 패닉으로 인해 비정상 종료되면, 이 Notifier는 더 이상
			// 메시지를 발송할 수 없는 '좀비 상태'가 됩니다.
			//
			// 문제 시나리오 (Close()를 호출하지 않는 경우):
			//   1. Sender 고루틴이 패닉으로 종료됨
			//   2. 하지만 Notifier의 상태는 여전히 'Running'으로 남아있음
			//   3. 외부(Service)는 정상 동작 중이라고 착각하여 계속 Send() 호출
			//   4. Send()는 성공한 것처럼 보이지만(채널에 넣기만 함)
			//   5. 실제로는 아무도 채널에서 꺼내가지 않으므로 메시지가 유실됨
			//   6. 버퍼(1000개)가 가득 차면 Send()가 블로킹되어 전체 서비스 마비
			//
			// 해결 방법:
			//   Close()를 명시적으로 호출하여 상태를 'Closed'로 변경합니다.
			//   이후 Send() 호출 시 즉시 에러를 반환하므로, 외부에서 문제를 인지하고
			//   적절한 조치(재시작, 알림 등)를 취할 수 있습니다.
			n.Close()
		}
	}()
	...
	for {
		// ─────────────────────────────────────────────────────────────────────
		// [테스트 전용 훅] 패닉 복구 메커니즘 검증
		// ─────────────────────────────────────────────────────────────────────
		// 이 코드는 테스트 환경에서만 동작하는 '패닉 시뮬레이션 훅'입니다.
		//
		// 목적:
		//   Sender 고루틴의 패닉 복구 로직(위의 defer-recover)이 제대로 동작하는지
		//   자동화된 테스트로 검증하기 위한 장치입니다.
		//
		// 동작 방식:
		//   - 프로덕션: testHookInjectSenderPanic은 항상 nil이므로 이 블록은 실행되지 않음
		//   - 테스트: sender_worker_test.go에서 이 필드에 panic() 함수를 주입하여
		//             의도적으로 패닉을 발생시키고, 복구 및 Close() 호출을 검증함
		//
		// 검증 대상:
		//   1. 패닉 발생 시 defer-recover가 정상 동작하는가?
		//   2. 패닉 후 n.Close()가 호출되어 상태가 'Closed'로 변경되는가?
		//   3. 에러 로그가 올바르게 기록되는가?
		if n.testHookInjectSenderPanic != nil {
			n.testHookInjectSenderPanic()
		}
