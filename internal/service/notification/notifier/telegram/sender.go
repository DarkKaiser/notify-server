...
func (n *telegramNotifier) handleNotifyRequest(ctx context.Context, notification *notifier.Notification) {
	...
	if notification.TaskContext != nil {
		message = n.enrichMessageWithContext(notification.TaskContext, message)
	}
...
func (n *telegramNotifier) appendCancelCommand(taskCtx contract.TaskContext, message string) string {
	...
	return fmt.Sprintf("%s\n\n%s%s%s%s", message, botCommandPrefix, botCommandCancel, botCommandSeparator, instanceID)
}
