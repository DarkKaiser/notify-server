...
// NewErrInvalidBotToken 제공된 Bot Token이 유효하지 않아 텔레그램 API 클라이언트 초기화에 실패했을 때 발생하는 에러를 생성합니다.
...
// NewErrInvalidCommandIDs 명령어 등록 과정에서 필수 식별자인 TaskID 또는 CommandID가 누락된 경우 발생하는 에러를 생성합니다.
...
// NewErrDuplicateCommandName 이미 등록된 명령어와 이름이 중복되는 충돌이 감지되었을 때 발생하는 에러를 생성합니다.
