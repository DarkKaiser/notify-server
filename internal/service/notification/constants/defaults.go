...
// TODO 미완료
const (
	// DefaultTelegramRetryDelay 알림 발송 실패 시 재시도 대기 시간의 기본값입니다.
	DefaultTelegramRetryDelay = 1 * time.Second

	// DefaultTelegramRateLimit 텔레그램 API Rate Limit 기본값 (초당 허용 요청 수)
	// 공식 문서는 채팅방당 초당 1회, 전역 초당 30회를 권장합니다.
	DefaultTelegramRateLimit = 1

	// DefaultTelegramRateBurst 텔레그램 API Rate Limit 버스트 기본값 (순간 최대 허용 요청 수)
	DefaultTelegramRateBurst = 5

	// DefaultTelegramHTTPClientTimeout 텔레그램 API 클라이언트의 HTTP 요청 타임아웃 기본값
	// Long Polling 타임아웃(60s)보다 넉넉하게 설정하여, 클라이언트가 먼저 연결을 끊는 문제를 방지합니다.
	DefaultTelegramHTTPClientTimeout = 70 * time.Second

	// DefaultTelegramEnqueueTimeout 알림 발송 요청 시 대기열이 가득 찼을 때 대기하는 최대 시간 (Backpressure)
	DefaultTelegramEnqueueTimeout = 5 * time.Second
	...
	// TelegramCommandExecutionLimit 텔레그램 봇 명령어의 최대 동시 처리 개수입니다.
	// 너무 높으면 시스템 리소스에 부담을 줄 수 있고, 너무 낮으면 사용자 반응이 지연될 수 있습니다.
	TelegramCommandExecutionLimit = 100
