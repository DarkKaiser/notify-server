...
//
//  8. BodyLimit - 요청 본문 크기 제한 (기본: 2MB, 초과 시 413 응답)
//     - 대용량 요청으로 인한 메모리 고갈 및 DoS 공격 방지
//
//  9. Timeout - 요청 처리 시간 제한 (기본: 60초, 초과 시 503 응답)
...
func NewHTTPServer(cfg HTTPServerConfig) *echo.Echo {
	...
	// 6. HTTP 로깅 (RateLimit/Timeout 이전에 위치하여 429/503 에러도 기록)
